	...
	<div id="main">
		...
		<div id="header">
			...
			<div id="menubar">
				<ul id="menu">
					...
					<li class="tab_selected"><a href="mostrar.php">ESPECÍMENES</a></li>
		...
		<div id="site_content">
			...
			<div id="content">
				...
				<p align="justify">Aquí podrá localizar y visualizar todos los datos de los especímenes que se encuentran depositados en nuestro herbario institucional.</p>
				<h2><strong>Búsqueda</strong></h2>
				<p align="justify">La búsqueda la puede realizar por parámetros de forma individual y si requeriré de una búsqueda más especifica coloque mayor cantidad de parámetros de búsqueda.</p>
				<form name="contact" method="POST" action="mostrar.php">
					<table class="uno" id="tablebonita" width="auto" cellpadding="2" cellspacing="5"  border="5" bordercolor="#5AAD8C">
						...
						<tr><td><strong>Taxon</strong></td><td><input class="contact" name="txtTaxon"   title="sin espacios en blanco al inicio o al final" autocomplete="on"/></td></tr>
				...
				<table id ="mostrar" class="display" align="center" bordercolor="#5AAD8C" border="5" style="width:auto" cellpadding="2" cellspacing="2">
					...
					if($vecTodos['resultado']){
						for($j=0; $j<count($vecTodos['respDatos']); $j++){
							...
							<td> <a href="mostrardetalle.php?variable1='.$vecTodos['respDatos'][$j]['idcartulina'].'" target="_blank"> '.$vecTodos['respDatos'][$j]['idcartulina'].'</a></td>
							<td><a href="mostrardetalle.php?variable1='.$vecTodos['respDatos'][$j]['idcartulina'].'" target="_blank">'.$vecTodos['respDatos'][$j]['indexautor'].'</a></td>
							<td><a href="mostrardetalle.php?variable1='.$vecTodos['respDatos'][$j]['idcartulina'].'" target="_blank">'.$vecTodos['respDatos'][$j]['idcoleccion'].'</a></td>
							<td><a href="mostrardetalle.php?variable1='.$vecTodos['respDatos'][$j]['idcartulina'].'" target="_blank">'.$vecTodos['respDatos'][$j]['family'].'</a></td>
							<td><a href="mostrardetalle.php?variable1='.$vecTodos['respDatos'][$j]['idcartulina'].'" target="_blank">'.$vecTodos['respDatos'][$j]['taxon'].'</a></td>
							<td><a href="mostrardetalle.php?variable1='.$vecTodos['respDatos'][$j]['idcartulina'].'" target="_blank">'.$vecTodos['respDatos'][$j]['provincia'].'</a></td>
							<td><a href="mostrardetalle.php?variable1='.$vecTodos['respDatos'][$j]['idcartulina'].'" target="_blank">'.$vecTodos['respDatos'][$j]['lower'].'</a></td>
							<td><a href="mostrardetalle.php?variable1='.$vecTodos['respDatos'][$j]['idcartulina'].'" target="_blank">'.$vecTodos['respDatos'][$j]['elevation'].'</a></td>
							</tr>';
						}
					}
					?>
		...
		<script type="text/javascript">
			...
			function verServicios() {
				$.ajax({
					type: 'GET',
					url: 'https://herbario.espoch.edu.ec/servicios/rutas/rutaSelect.php/?opc=verDatos&&cual=1',
					headers: { "x-requested-with": "xhr" },
					success: function (data) {
						var datos = JSON.parse(data);
						console.log(datos.respDatos);
					},
					error: function () {
						alertify.error("Error al procesar la información.");
					}
				});
			}
		</script>
